...
unsigned int inode_group_offset(int num_group, struct state* fs_state) {
    if (num_group == 0 || check_power(num_group, 3) == 1 || check_power(num_group, 5) == 1 || check_power(num_group, 7) == 1) {
        return num_group * fs_state->superBlock.s_blocks_per_group * fs_state->block_size + fs_state->groupDesc.bg_inode_table*fs_state->block_size;
    }
    return fs_state->block_size * num_group * fs_state->superBlock.s_blocks_per_group + 2 * fs_state->block_size;
}

unsigned int inode_offset(int num_group, int num_inode, struct state* fs_state) {
    if (num_group == 0 && num_inode < sys_inodes_count) {
            return num_inode * fs_state->superBlock.s_inode_size/2;
    }else{
        return (num_inode - 1) * fs_state->superBlock.s_inode_size;
    }
}
...
void read_inode(struct state* fs_state, int64_t inode_num, struct ext_inode* inode) {
    ...
    unsigned  int offset_block = inode_group_offset(num_group, fs_state) + inode_offset(num_group, inode_num, fs_state);
...
char* read_block_data(struct state* fs_state, struct ext_inode* inode, int index) {
    int fd;
    fd = open(fs_state->device_path, O_RDONLY);
