...
char *ls(FileSystem *fileSystem, char *path) {
    NodeInfo *info;
    char *output;
    if (path == NULL) {
...
char *pwd(FileSystem *fileSystem) {
...
char *cd(FileSystem *fileSystem, char *path) {
...
char *cp(FileSystem *fileSystem, char *path, char *outPath) {
    char *output = malloc(1);
    output[0] = '\0';
